...
WORK_IMAGE="$(gbs_prepare_work_image "${SRC_IMAGE}" "${WORK_DIR}" "originaltests")"

emit_summary() {
  local result="$1"
  local code="$2"
  local json_payload=""
  echo "ORIGINAL_TESTS_SUMMARY result=${result} code=${code} reload=${RELOAD_STATUS} unit=${UNIT_STATUS} preflight=${PREFLIGHT_STATUS} live=${LIVE_STATUS} work_image=${WORK_IMAGE}"
  if [[ "${JSON_SUMMARY}" == "1" ]]; then
    ...
    printf 'ORIGINAL_TESTS_SUMMARY_JSON %s\n' "${json_payload}"
    gbs_write_json_summary_file "original-tests-summary.json" "${json_payload}"
  fi
  gbs_append_summary_line "### Original-Tests Lane"
...
reload_output="$(HOME=/tmp/pharo-clean-auto/home GBS_WORK_IMAGE="${WORK_IMAGE}" GBS_LOAD_GROUP='Original-Tests' GBS_VERIFY_LANE='original-tests' "${VM}" --headless "${WORK_IMAGE}" st "/Users/tariq/src/gemtools/GemStone-Pharo-Bridge/scripts/clean_reload_gemstone.st" 2>&1 || true)"
echo "${reload_output}"
if grep -q "LOAD_ERROR" <<< "${reload_output}" || grep -q "NO_COMPAT_SOURCE_SCAN_FAILED" <<< "${reload_output}" || grep -q "NO_COMPATIBILITY_PROOF_FAILED" <<< "${reload_output}" || grep -q "ARCHITECTURE_BOUNDARY_FAILED" <<< "${reload_output}" || grep -q "PACKAGE_OWNERSHIP_DRIFT_FAILED" <<< "${reload_output}"; then
  ...
  emit_summary "FAIL" "ORIGINAL_TESTS_RELOAD_FAILED"
  echo "Original-tests clean reload failed." >&2
  exit 1
fi
RELOAD_STATUS="ok"
...
if grep -q "ORIGINAL_TESTS_SUMMARY result=FAIL" <<< "${reload_output}" || ! grep -q "ORIGINAL_TESTS_CHECK_OK" <<< "${reload_output}"; then
  emit_summary "FAIL" "$(extract_summary_code "${reload_output}")"
  echo "Original-tests verification run failed." >&2
  exit 1
fi

emit_summary "OK" "ORIGINAL_TESTS_CHECK_OK"
echo "ORIGINAL_TESTS_CHECK_OK"
