...
WORK_DIR="${2:-${SRC_DIR}}"
WORK_IMAGE="${WORK_DIR}/${SRC_BASE} - compatibility.image"
...
reload_output="$(HOME=/tmp/pharo-clean-auto/home GBS_LOAD_GROUP='Compatibility-Tests' "${VM}" --headless "${WORK_IMAGE}" st "/Users/tariq/src/gemtools/GemStone-Pharo-Bridge/scripts/clean_reload_gemstone.st" 2>&1 || true)"
echo "${reload_output}"
if grep -q "LOAD_ERROR" <<< "${reload_output}" || grep -q "COMPATIBILITY_DRIFT_FAILED" <<< "${reload_output}" || grep -q "ARCHITECTURE_BOUNDARY_FAILED" <<< "${reload_output}" || grep -q "PACKAGE_OWNERSHIP_DRIFT_FAILED" <<< "${reload_output}"; then
  echo "Compatibility clean reload failed." >&2
  exit 1
fi

unit_output="$(HOME=/tmp/pharo-clean-auto/home GBS_LOAD_GROUP='Compatibility-Tests' GBS_TEST_PACKAGES='GemStone-Pharo-Compatibility-Tests' GBS_TEST_LANE=unit "${VM}" --headless "${WORK_IMAGE}" st "/Users/tariq/src/gemtools/GemStone-Pharo-Bridge/scripts/run_all_package_regressions.st" 2>&1 || true)"
echo "${unit_output}"
if grep -q "LOAD_ERROR" <<< "${unit_output}" || grep -q "BRIDGE_UNIT_REGRESSION_FAILED" <<< "${unit_output}"; then
  echo "Compatibility regression run failed." >&2
  exit 1
fi

echo "BRIDGE_COMPATIBILITY_CHECK_OK"
