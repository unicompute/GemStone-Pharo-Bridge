...
if [[ "${RESULT}" == "OK" && "${CODE}" == "MATERIALIZATION_PERF_OK" ]]; then
  ...
  gbs_append_summary_line "- structured byte transport: shallow GCI-byte fetches \`${SHALLOW_STRUCTURED_BYTE_FETCHES}\`, shallow traversal-byte fetches \`${SHALLOW_STRUCTURED_TRAVERSAL_BYTE_FETCHES}\`, shallow fallbacks \`${SHALLOW_STRUCTURED_BYTE_FALLBACKS}/${SHALLOW_STRUCTURED_BYTE_FALLBACKS_MAX}\`, mixed GCI-byte fetches \`${MIXED_STRUCTURED_BYTE_FETCHES}\`, mixed traversal-byte fetches \`${MIXED_STRUCTURED_TRAVERSAL_BYTE_FETCHES}\`, mixed fallbacks \`${MIXED_STRUCTURED_BYTE_FALLBACKS}/${MIXED_STRUCTURED_BYTE_FALLBACKS_MAX}\`"
