...
- `make replication-live` validates connector install, connector-manager report counts, server clamp synchronization, callback clamp selector synchronization, per-instvar clamp entries, inherited named replication specs, a same-shape live migration-domain order/customer fixture with forwarder/min/max/stub/nil slot policy, a remapped live migration-domain fixture with VisualWorks-style `instVarMap` server-to-client offsets, client-only `nil` mapped slot mutation that does not dirty the replicate, native named-slot dirty writes for both domain fixtures, clamped traversal, native dirty-store flush, lifecycle queued/acknowledged export-set counters, a twelve-object business-write dirty traversal fixture, export-set queue acknowledgement, and trend thresholds against a live GemStone session.
...
- Per-instvar traversal semantics are live-validated for a five-slot same-shape order fixture and a five-slot remapped order fixture with an unmapped server slot. Strict application parity should still be checked against real callback methods, inherited maps, and domain-specific `instVarMap` methods from migration projects.
