...
- Migration-domain fixtures that create live GemStone order/customer classes, install Pharo domain connectors, verify forwarder/min/max/stub/nil slot policy, verify the server callback clamp selector, execute a dynamic callback-driven clamped traversal, and flush named-slot dirty writes through native store traversal buffers. The remapped fixtures use VisualWorks-style `instVarMap` definitions where server slots are reordered, server-only slots are unmapped, client-only `nil` mapped slots do not dirty the replicate, and an inherited `super instVarMap , ...` subclass map extends the same behavior. The native dirty-store path preserves unmapped server slots while writing mapped client slots in server order.
- Mapped-replicator lifecycle checks that refresh class-version metadata, assert updated server-to-client offsets and clamp levels, and verify live commit, abort, logout, and reconnect paths clear dirty materialized state without retaining stale mapped replicators.
...
Thresholds are configured through `scripts/replication_live_thresholds.env` or direct `GBS_REPLICATION_LIVE_*` environment variables. The defaults require at least one callback clamp spec, at least one per-instvar clamp entry, at least one inherited replication-spec fixture, at least three migration-domain fixtures, at least one domain callback clamp spec, at least four domain per-instvar clamp entries, at least six domain dirty objects flushed through native dirty-store buffers, at least three domain native dirty-store flushes, at least six domain native dirty-store entries, at least one clamped traversal fetch, zero clamped fallbacks, at least one native dirty-store flush, at least one semantic dirty-store command, at least three semantic Dictionary/Set/Bag entries, at least six mixed dirty objects flushed, at least twelve business dirty objects flushed, and zero export-set release queue entries remaining after both flushes. Trend regression uses `GBS_REPLICATION_LIVE_REGRESSION_PERCENT` or `GBS_REPLICATION_LIVE_REGRESSION_MIN_DELTA_MS`, whichever allows the larger drift over the previous persisted sample.
